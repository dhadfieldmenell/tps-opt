...
__global__ void _normProbNM(float* prob_nm, int* xdims, int* ydims,
  ...
  if (threadIdx.x == 0) xdim = xdims[blockIdx.x] + 1;
  if (threadIdx.x == 1) ydim = ydims[blockIdx.x] + 1;
...
__global__ void _getTargPts(float* x, float* y, float* xw, float*yw,
  ...
  if (tix < xdim){
    for(int i = 0; i < DATA_DIM; ++i){
      s_x[rMInd(tix, i, DATA_DIM)] = x[rMInd(d_offset, tix, i, DATA_DIM)];
    }
  }
  if (tix < ydim){
    for(int i = 0; i < DATA_DIM; ++i){
      s_y[rMInd(tix, i, DATA_DIM)] = y[rMInd(d_offset, tix, i, DATA_DIM)];
    }
  }
